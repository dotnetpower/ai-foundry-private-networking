...
│        ┌────────────┼────────────┬──────────┐          │
│   ┌────▼────┐ ┌─────▼─────┐ ┌───▼──────┐ ┌──▼───────┐ │
│   │ Storage │ │ Key Vault │ │  OpenAI  │ │AI Search │ │
│   │ Account │ │           │ │ (GPT-4o) │ │(RAG검색) │ │
│   └─────────┘ └───────────┘ └──────────┘ └──────────┘ │
└───────────────────────────────────────────────────────┘
```
...
| AI Hub | `amlworkspace` | `privatelink.api.azureml.ms` |
...
| `dependent-resources/main.bicep` → AI Search | Step 6-2 | Basic SKU, `publicNetworkAccess: Disabled` |
| `ai-foundry/main.bicep` → RBAC | Step 10 | Hub/Project/Search MI 역할 |
| `ai-foundry/main.bicep` → Search Connection | Step 8 | AAD 인증, Hub Managed VNet PE |
| `private-endpoints/main.bicep` → PE | Step 11 | 6개 PE (Search 포함) |
...
# AI Search (Bicep에서 이미 Disabled로 배포됨 — 확인용)
...
### Azure Policy로 publicNetworkAccess 거부 시 배포 실패

**증상**: 배포 시 `RequestDisallowedByPolicy` 오류 발생. Storage, Key Vault, OpenAI, Hub 리소스에서 `publicNetworkAccess: Enabled` 설정이 정책에 의해 거부됨.

**원인**: 구독 또는 관리 그룹에 `publicNetworkAccess`를 `Disabled`로 강제하는 Azure Policy(`Deny`)가 적용되어 있음. Hub Managed VNet은 Storage/KV/OpenAI에 내부 PE를 **배포 시점에 자동 프로비저닝**하므로 초기에 `Enabled`가 필수 — 이는 Azure 플랫폼의 구조적 제약임.

> **참고**: AI Search는 Hub Managed VNet PE가 `outboundRules`에서 별도 생성되므로 배포 시점부터 `Disabled` 가능 (현재 Bicep에 반영 완료).

**해결 방법 1: Policy Exemption (권장)**

배포 RG에 한시적(2시간) 예외를 부여하고, 배포 + private 전환 완료 후 제거:

```bash
# 1. 정책 할당 ID 확인
az policy assignment list --scope "/subscriptions/<sub-id>" \
  --query "[?contains(displayName,'public')].{name:name, id:id}" -o table

# 2. 배포 RG에 한시적 예외 생성 (2시간 후 자동 만료)
az policy exemption create \
  --name "aif-deploy-exemption" \
  --policy-assignment "<정책 할당 ID>" \
  --scope "/subscriptions/<sub-id>/resourceGroups/<rg-name>" \
  --exemption-category "Waiver" \
  --expires-on "$(date -u -d '+2 hours' +%Y-%m-%dT%H:%M:%SZ)"

# 3. Bicep 배포 실행
az deployment sub create --location swedencentral \
  --template-file main.bicep --parameters parameters/dev2.bicepparam

# 4. 배포 완료 후 public disable 전환 (위 '보안 강화' 섹션 참고)

# 5. 예외 제거 (또는 만료까지 대기)
az policy exemption delete --name "aif-deploy-exemption" \
  --scope "/subscriptions/<sub-id>/resourceGroups/<rg-name>"
```

**해결 방법 2: Policy 조건 수정 (정책 관리자 요청)**

Policy 정의에서 `Microsoft.MachineLearningServices` 리소스 프로바이더 또는 특정 RG를 예외 조건으로 추가하도록 정책 관리자에게 요청합니다.

**해결 방법 3: 아키텍처 변경 (비권장)**

Managed VNet을 사용하지 않고 직접 VNet에 Hub를 배포하면 모든 PE를 수동 생성할 수 있어 `Disabled`로 시작 가능합니다. 단, Hub 내부 컴퓨팅(Compute Instance, Serverless 등) 네트워크 관리가 복잡해집니다.
